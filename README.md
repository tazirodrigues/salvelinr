...
- `season` requires a vector of the first days of seasons. **It currently only works for one calendar year at a time, and it assumes four seasons.** If type == "ice" it will assign based on dates, and if type == "month" it will assign based on months. The hemisphere argument defaults to "north," but specifying hemisphere = "south" will flip the seasonal order.
